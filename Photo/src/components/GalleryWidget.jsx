...
const GalleryWidget = () => {
  ...
  const handleImageUpload = (e) => {
    ...
    if (file) {
      ...
      setImages([...images, imageUrl]); // Add the new image to the gallery
    }
  };
  ...
  return (
    <div className="bg-slate-800 p-4 sm:p-6 md:p-8 lg:p-10 rounded-lg shadow-md">
      <h2 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-semibold mb-4 relative bg-black rounded mx-5 text-white text-center hover:bg-slate-500">
        Gallery
      </h2>
      <div className="flex flex-col sm:flex-row items-center justify-between">
        ...
        />
        <div className="flex flex-col sm:flex-row items-center justify-center sm:mx-4">
          <button
            className="bg-gray-800 text-white py-2 px-4 rounded mx-4 mb-4 sm:mb-0 hover:bg-slate-600 shadow-slate-800"
            onClick={() => document.getElementById("fileInput").click()}
          >
            + Add Image
          </button>
          {/* Arrow buttons on the right end */}
          <div className="flex space-x-2">
            {/* Left arrow */}
            <button
              className="bg-gray-800 text-white rounded-full p-2 hover:bg-slate-500"
              onClick={handlePrev}
            >
              &#8592;
            </button>
      ...
      />
      {/* Display all the added photos below */}
      <div className="flex flex-wrap mt-4">
        {images.map((image, index) => (
          <img
            ...
            className="w-24 h-24 sm:w-32 sm:h-32 rounded-lg m-2"
          />
        ))}
      </div>
    </div>
